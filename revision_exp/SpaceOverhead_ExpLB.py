...
class Bounded1NN(object):
    ...
    def __init__(self, metric, lb = True, constraint = None, w = 10000, epsilon = 0.2, m = 0, g = 0.3, c =0.5, lamb =1, nu = 0.0001, timesx = None, timesy = None, p = 5, r = 1, precomupted = 1):
        self.metric = metric
        self.w = w
    ...
    def predict(self, Y):
        ...
        if self.lb == True: 
            ...
            if self.metric in ["GLB_DTW", 
                               ...
                               "Breakdown_QueryData",
                               "Cas_Keogh_GLB"]:
                XUE, XLE = make_envelopes(self.X[0: round(len(self.X) * self.precomputed)], window)
                #self.XUE, self.XLE = make_envelopes(self.X[0: round(len(self.X) * PRECOMPUTED_PCT)], window)
                self.XUE = np.copy(XUE)
                self.XLE = np.copy(XLE)

            if self.metric in ["LB_Keogh", "GLB_DTW", "ddtw", "LB_Improved", "Cas_Keogh_New", "Cas_Keogh_Improved","Cas_Keogh_GLB", 
                                "GLB_ERP", "LB_Keogh_ERP", 
                                ...
                                "LB_TWED", "GLB_TWED", 
                                "Breakdown_QueryData", 
                                "Breakdown_QueryOnly", "Breakdown_QueryBoundary"]:
            ...
            if self.metric == "Cas_Keogh_GLB":
                ...
                self.XUEC[0: round(len(self.X) * self.precomputed)] = np.copy(XUE)
                self.XLEC[0: round(len(self.X) * self.precomputed)] = np.copy(XLE)
            ...
            for idx_y, y in enumerate(Y):
                ...
                for idx_x, x in enumerate(self.X):
                    ...
                    if self.metric == "GLB_DTW" and idx_x > len(self.XUE)-1:
                        new_x = np.array([x])
                        new_XUE, new_XLE  = make_envelopes(new_x, window)


                        self.XUE = np.concatenate((self.XUE, new_XUE), axis = 0)
                        self.XLE = np.concatenate((self.XLE, new_XLE), axis = 0)
                ...
                for idx_x, x in enumerate(self.X):
                    ...
                    if self.metric == "Cas_Keogh_GLB" and lb_dist < best_so_far:
                        ...
                        if sum(abs(self.XUEC[idx_x])) == 0 and sum(abs(self.XLEC[idx_x])) == 0:

                            UE, LE = make_envelopes(np.array([x]), window)
