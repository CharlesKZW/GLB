...
for d in dataset_test:
    ...
    for algo in algo_list:
        ...
        ticc = time.time()
        if algo == "GLB_EDR":
            m = Bounded1NN(metric = "{}".format(algo), lb = True, constraint = "Sakoe-Chiba", w = 0.05, epsilon = 0.1, m = 0, g = 0.3, c =0.5, lamb =1, nu = 0.0001, timesx = None, timesy = None, p = 5, r = 1)
        else:
            m = Bounded1NN(metric = "{}".format(algo), lb = True, constraint = "Sakoe-Chiba", w = 0.05, epsilon = 0.2, m = 0, g = 0.3, c =0.5, lamb =1, nu = 0.0001, timesx = None, timesy = None, p = 5, r = 1)
        ...
        tocc = time.time()
        dataset_result['Bounded1NN_{}_acc'.format(algo)] = lb_acc
        dataset_result['Bounded1NN_{}_pruning'.format(algo)] = pruning_power
        dataset_result['Bounded1NN_{}_runtime'.format(algo)] = tocc - ticc

        
        ticc = time.time()
        if algo == "GLB_EDR":
            m = Bounded1NN(metric = "{}".format(algo), lb = False, constraint = "Sakoe-Chiba", w = 0.05, epsilon = 0.1, m = 0, g = 0.3, c =0.5, lamb =1, nu = 0.0001, timesx = None, timesy = None, p = 5, r = 1)
        else:
            m = Bounded1NN(metric = "{}".format(algo), lb = False, constraint = "Sakoe-Chiba", w = 0.05, epsilon = 0.2, m = 0, g = 0.3, c =0.5, lamb =1, nu = 0.0001, timesx = None, timesy = None, p = 5, r = 1)
        m.fit(dict["{}_train_X".format(d)], dict["{}_train_y".format(d)])
        ...
        lb_acc = (dict["{}_test_y".format(d)].shape[0] - lb_wrong_predict_count) / dict["{}_test_y".format(d)].shape[0]
        tocc = time.time()
        dataset_result['1NN_{}_acc'.format(algo)] = lb_acc
       
        dataset_result['1NN_{}_runtime'.format(algo)] = tocc - ticc
        
        dataset_result['{} Speed Up'.format(algo)] = dataset_result['1NN_{}_runtime'.format(algo)] / dataset_result['Bounded1NN_{}_runtime'.format(algo)]
        dataset_result['{} acc_diff'.format(algo)] = dataset_result['Bounded1NN_{}_acc'.format(algo)] - dataset_result['1NN_{}_acc'.format(algo)]

    results['{}'.format(d)] = dataset_result

df = pd.DataFrame.from_dict(results, orient = 'index')
df.to_excel("Exp.xlsx")
